...
Injector.prototype.loadPackages = function (dir, done) {
  ...
  if (this.packageNamePrefix) {
    rmPrefix = new RegExp('^' + this.packageNamePrefix + '-');
  }
        ...
        if (stats.isDirectory()) {
          ...
          fs.stat(path.join(file, PACKAGE_FILE), function (err, stats) {
            ...
            if (stats.isFile()) {
              ...
              if (rmPrefix) {
                pkg.name = pkg.name.replace(rmPrefix, '');
              }

              pkg.injector = that;
              if (!!~pkg.keywords.indexOf(that.keyword)) {
                pkgs[pkg.name] = pkg;
              }
            }
            complete();
...
Injector.prototype.init = function (done) {
  ...
  (function () {
    ...
    (function next() {
      ...
      that.dependencies[camel(pkg.name)] = pkg;
      pkg.inject(pkg.wrapper);

      pkg.emit('init', that.config.namespace('_.config.' + pkg.name), function (err) {
        // wrapper 執行失敗時停止繼續執行
        if (err) {
          return done(err);
        }
